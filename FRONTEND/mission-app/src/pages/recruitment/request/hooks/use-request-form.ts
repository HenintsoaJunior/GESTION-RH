...
const useMissionForm = ({
  ...
  const updateMutation = useUpdateMission(requestId || "");
  ...
  useEffect(() => {
    if (!isOpen || !requestId || !missionResponse?.data || regions.length === 0 || employeeSuggestions.length === 0 || transportSuggestions.length === 0) return;
  ...
  }, [isOpen, requestId, missionResponse, regions, employeeSuggestions, transportSuggestions]);
