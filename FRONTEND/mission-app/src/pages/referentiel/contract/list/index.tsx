  ...
  const handleConfirmDelete = useCallback(() => {
    if (contractTypeToDelete) {
      deleteContractTypeMutation.mutate(contractTypeToDelete, {
        onSuccess: () => {
          setAlert({ isOpen: true, type: "success", message: "Type de contrat supprimé avec succès." });
          setIsDeleteModalOpen(false);
          refetch();
        },
        onError: () => {
          setAlert({ isOpen: true, type: "error", message: "Erreur lors de la suppression du type de contrat." });
        },
      });
    }
          ...
          <tbody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={4}>
                  <NoDataMessage>
                    {hasFilters ? "Aucun type de contrat ne correspond aux critères." : "Aucun type de contrat trouvé."}
                  </NoDataMessage>
                </TableCell>
              </TableRow>
            )}
