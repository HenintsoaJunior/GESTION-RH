...
const ExpenseReportStep = ({
  ...
  useEffect(() => { setHasLoadedReports(false); }, [formData.assignationId]);
  ...
  }, [expenseLinesByType, handleInputChange, defaultFields]);
  ...
  }, [expenseLinesByType, handleInputChange]);
  ...
  }, [currencyCodes, handleLineInputChange, getLineError, isSubmitting]);
  ...
  const handleSubmitExpenseReport = useCallback(async () => {
    ...
    if (!formData.assignationId) return setAlert({ isOpen: true, type: "error", message: "ID d'assignation manquant." });
  ...
  }, [isSubmitting, isLoading, formData, expenseLinesByType, attachments, existingAttachments, createExpenseReport, handleInputChange, refetchExpenseReports, onSubmitSuccess]);
    ...
    <>
            ...
            <AccordionContentStyled $isOpen={isOpen}>
              {fieldsForType.length > 0 ? (
                      ...
                      {currentLines.length > 0 ? (
                        currentLines.map((line: any, lineIndex: number) => (
                          <tr key={lineIndex} style={{ backgroundColor: lineIndex % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)" }}>
                            ...
                            <FormFieldCell style={{ textAlign: "center" as const, padding: "0.75rem", border: "1px solid var(--border-color)" }}>
                              <RemoveItem type="button" onClick={() => handleRemoveLine(typeId, lineIndex)} disabled={isSubmitting}><Trash2 size={16} /></RemoveItem>
                            </FormFieldCell>
                          </tr>
                        ))
                      ) : (
                        <tr style={{ backgroundColor: "var(--info-bg)" }}>
                          <td colSpan={fieldsForType.length + 1} style={{ textAlign: "center" as const, padding: "1.5rem", fontStyle: "italic" as const, color: "var(--text-secondary)", border: "1px solid var(--border-color)" }}>
                            <Button type="button" onClick={() => handleAddLine(typeId, fieldsForType)} disabled={isSubmitting} style={{ background: "var(--primary-color)", color: "var(--text-white)", padding: "8px 16px", borderRadius: "4px", fontSize: "0.875rem" }}>
                ...
                <div style={{ padding: "1rem", textAlign: "center" as const, color: "var(--text-secondary)" }}>
                  ...
                  <Button type="button" onClick={() => handleAddLine(typeId, defaultFields)} disabled={isSubmitting}>Ajouter une ligne avec champs par défaut</Button>
                </div>
              )}
      ...
      <AttachmentSection>
        ...
        >
          ...
          <p style={{ fontSize: "0.875rem", color: "var(--text-color)", marginBottom: "0.25rem" }}>Glissez-déposez vos justificatifs ici</p>
          <p style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginBottom: "0.5rem" }}>ou cliquez pour parcourir vos fichiers (PDF, images, documents)</p>
         
          {isSubmitting && <p style={{ color: "var(--danger-color)", fontSize: "0.625rem" }}>Téléversement désactivé pendant la soumission</p>}
        </div>
        {totalAttachments > 0 && (
          <AttachmentsList>
                ...
                {existingAttachments.map((file: Attachment, index: number) => (
                  <AttachmentItem key={`existing-${index}`} style={attachmentItemStyle(true)}>
                    ...
                    <div style={actionStyle}>
                      ...
                      <IconButton type="button" onClick={() => setExistingAttachments(prev => prev.filter((_: Attachment, i: number) => i !== index))} disabled={isSubmitting} title="Supprimer" style={{ background: 'var(--danger-color)', color: 'var(--text-white)' }}><Trash2 size={14} /></IconButton>
                    </div>
                  </AttachmentItem>
                ))}
              </AttachmentCategory>
                ...
                {attachments.map((file: Attachment, index: number) => (
                  <AttachmentItem key={`new-${index}`} style={attachmentItemStyle(false)}>
                    ...
                    <div style={actionStyle}>
                      <IconButton type="button" onClick={() => handleFileView(file.fileContent, file.fileName, setModalContent, setModalOpen, file.fileType)} title="Prévisualiser" style={{ background: "var(--bg-secondary)" }}><Eye size={14} /></IconButton>
                      <IconButton type="button" onClick={() => handleFileDownload(file.fileContent, file.fileName)} title="Télécharger" $variant="primary"><Download size={14} /></IconButton>
                      <IconButton type="button" onClick={() => handleRemoveAttachment(index)} disabled={isSubmitting} title="Supprimer" style={{ background: 'var(--danger-color)', color: 'var(--text-white)' }}><Trash2 size={14} /></IconButton>
                    </div>
                  </AttachmentItem>
                ))}
              </AttachmentCategory>
      ...
      <FormActions style={{ justifyContent: "center" as const, marginTop: "2rem" }}>
        ...
        >
          {isLoading ? <><Loader2 size={18} style={{ animation: "spin 1s linear infinite" }} /> Soumission en cours...</> : <><Upload size={18} /> Soumettre le Rapport de Frais</>}
        </Button>
      </FormActions>
